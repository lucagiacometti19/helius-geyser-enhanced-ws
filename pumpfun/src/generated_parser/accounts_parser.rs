...
impl PumpProgramState {
    pub fn try_unpack(data_bytes:&[u8]) -> yellowstone_vixen_core::ParseResult<Self>{
            ...
            let acc = match acc_discriminator {
                                                [23, 183, 248, 55, 96, 216, 172, 96] => Ok(
                        PumpProgramState::BondingCurve(
                            deserialize_checked(data_bytes, &acc_discriminator)?
                        )
                    ),
                                                                [143, 52, 146, 187, 219, 123, 76, 155] => Ok(
                        PumpProgramState::FeeConfig(
                            deserialize_checked(data_bytes, &acc_discriminator)?
                        )
                    ),
                                                                [167, 232, 232, 177, 200, 108, 114, 127] => Ok(
                        PumpProgramState::Global(
                            deserialize_checked(data_bytes, &acc_discriminator)?
                        )
                    ),
                        ...
                        PumpProgramState::GlobalVolumeAccumulator(
                            deserialize_checked(data_bytes, &acc_discriminator)?
                                                                ...
                                                                [86, 255, 112, 14, 102, 53, 154, 250] => Ok(
                        PumpProgramState::UserVolumeAccumulator(
                            deserialize_checked(data_bytes, &acc_discriminator)?
                        )
                    ),
    ...
    impl IntoProto<proto_def::ProgramState> for PumpProgramState {
        fn into_proto(self) -> proto_def::ProgramState {
            let state_oneof = match self {
                                    PumpProgramState::BondingCurve(data) => proto_def::program_state::StateOneof::BondingCurve(data.into_proto()),
